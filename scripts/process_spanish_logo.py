#!/usr/bin/env python3
"""
Process the Spanish logo and generate all required icon sizes
Adds the purple-blue gradient background matching the English version
"""
...
def create_icon_with_background(source_img, size):
    ...
    # Create purple-blue gradient (matching English version)
    for y in range(size):
        progress = y / size
    ...
    # Composite logo on background (if logo has transparency)
...
def generate_all_sizes():
    ...
    print("Generating Spanish app icons...")
    ...
    print(f"\n✅ Generated {len(sizes)} Spanish icon files")
