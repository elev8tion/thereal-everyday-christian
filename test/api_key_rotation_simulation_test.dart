...
void main() {
  group('API Key Rotation Simulation Tests', () {
    ...
    test('Random starting position is within valid range', () async {
      ...
      expect(
          startingPositions.every((pos) => pos >= 0 && pos < poolSize), isTrue);
      ...
      print(
          '✅ Random starting positions: ${uniquePositions.length} unique positions from 100 users');
    });
    ...
    test('Distribution is even over 1000 launches (single user)', () async {
      ...
      final maxDeviation = keyUsageCounts
          .map((count) => (count - expectedUsagePerKey).abs())
          .reduce((a, b) => a > b ? a : b);
      ...
      for (int i = 0; i < poolSize; i++) {
        final percentage =
            (keyUsageCounts[i] / launchCount * 100).toStringAsFixed(1);
        final bar = '█' * (keyUsageCounts[i] ~/ 2);
        print(
            'Key #${(i + 1).toString().padLeft(2)}: ${keyUsageCounts[i].toString().padLeft(3)} uses ($percentage%) $bar');
      }
      ...
      print(
          '✅ Distribution verified: All keys used, average close to expected');
    });
    ...
    test('Multiple users have desynchronized starting positions', () async {
      ...
      for (int i = 0; i < poolSize; i++) {
        final percentage =
            (startingPositionCounts[i] / userCount * 100).toStringAsFixed(1);
        final bar = '█' * startingPositionCounts[i];
        print(
            'Position ${(i).toString().padLeft(2)}: ${startingPositionCounts[i].toString().padLeft(2)} users ($percentage%) $bar');
      }

      // Verify positions are spread out
      final avgUsersPerPosition =
          startingPositionCounts.reduce((a, b) => a + b) / poolSize;
      print(
      ...
      expect(avgUsersPerPosition, closeTo(expectedPerPosition, 2.0),
          reason:
              'Users should be relatively evenly distributed across starting positions');

      print(
          '✅ Starting positions are desynchronized (good for preventing concurrent spikes)');
    });
    ...
    test('Concurrent users at peak hour (10,000 users simulation)', () async {
      ...
      for (int userId = 0; userId < userCount; userId++) {
        ...
        final keyIndex = await simulateGetApiKey(
            poolSize: poolSize, counterKey: 'user_$userId');
      ...
      for (int i = 0; i < poolSize; i++) {
        final percentage =
            (currentKeyUsage[i] / userCount * 100).toStringAsFixed(1);
        final bar = '█' * (currentKeyUsage[i] ~/ 25);
        print(
            'Key #${(i + 1).toString().padLeft(2)}: ${currentKeyUsage[i].toString().padLeft(4)} users ($percentage%) $bar');
      }
      ...
      const requestsPerKey = expectedUsersPerKey * messagesPerUser;
      const capacityPerKeyPerHour = 3600; // 60 req/min × 60 min
      final capacityUsagePercent =
          (requestsPerKey / capacityPerKeyPerHour * 100).toStringAsFixed(1);
      ...
      print(
          '✅ Peak load distributed well: $capacityUsagePercent% average capacity usage');
    });
    ...
    test('Long-term distribution (100 users × 50 launches each)', () async {
      ...
      for (int i = 0; i < poolSize; i++) {
        final percentage =
            (totalKeyUsage[i] / totalLaunches * 100).toStringAsFixed(1);
        final deviation =
            (totalKeyUsage[i] - expectedUsagePerKey).toStringAsFixed(1);
        final deviationSign = totalKeyUsage[i] > expectedUsagePerKey ? '+' : '';
        final bar = '█' * (totalKeyUsage[i] ~/ 10);
        print(
            'Key #${(i + 1).toString().padLeft(2)}: ${totalKeyUsage[i].toString().padLeft(4)} ($percentage%) $deviationSign$deviation $bar');
      }

      final avgUsage = totalKeyUsage.reduce((a, b) => a + b) / poolSize;
      final maxDeviation = totalKeyUsage
          .map((count) => (count - expectedUsagePerKey).abs())
          .reduce((a, b) => a > b ? a : b);
